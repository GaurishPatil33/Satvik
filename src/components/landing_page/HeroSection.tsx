      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 w-full flex-1 flex items-center pt-32 pb-20">
        <div className={`max-w-xl transition-all duration-500 ${animating ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'}`}>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button className="px-7 py-3.5 rounded-full text-sm font-body font-semibold text-bark border border-bark/20 hover:border-bark/50 hover:bg-cream-dark transition-all duration-300 flex items-center gap-2">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 w-full pb-8 flex items-center justify-between">
        <div className="flex gap-2">
          {slides.map((_, i) => (
            <button
              key={i}
              onClick={() => go(i)}
              className={`transition-all duration-300 rounded-full ${i === current ? 'w-8 h-2 bg-forest' : 'w-2 h-2 bg-bark/20 hover:bg-bark/40'
                }`}
            />
          ))}
        </div>
        <div className="flex gap-2">
          <button
            ...
            className="w-10 h-10 rounded-full border border-bark/20 flex items-center justify-center hover:bg-cream-dark transition-colors"
          >
            <ChevronLeft size={18} />
          </button>
          <button
            onClick={() => go((current + 1) % slides.length)}
            className="w-10 h-10 rounded-full bg-forest text-cream flex items-center justify-center hover:bg-bark-light transition-colors"
          >
