...
export function filterAndSort(products: Product[], filters: FilterState): Product[] {
  ...
  data = data.filter(p => p.price >= filters.priceMin && p.price <= filters.priceMax);
  ...
  switch (filters.sort) {
    ...
    case "newest": data.sort((a, b) => (b.isNew ? 1 : 0) - (a.isNew ? 1 : 0)); break;
    case "discount": data.sort((a, b) => b.discount - a.discount); break;
    default: data.sort((a, b) => b.reviews - a.reviews);
  }
